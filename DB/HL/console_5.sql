...
(
    ...
    regdate    date default sysdate NOT NULL,
    memberSeq  number references tblMember (memberSeq),
    routinesetSeq number references tblRoutineSet(routinesetSeq)
);
...
(
    ...
    kind            varchar2(200)        NOT   NULL check (kind in ('루틴', '챌린지')),     --루틴or챌린지
    rest            varchar2(200)        NULL check (rest in ('Y', 'N')),
    time            varchar2(100)       NOT  NULL check (time in ('아침', '오후', '저녁')),--아침, 오후, 저녁
...
(
    ...
    routinesetcontent  varchar2(1000),
    routinesetname      varchar2(500)
